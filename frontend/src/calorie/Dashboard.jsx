...
const Dashboard = () => {
    ...
    const getBarData = (monthStr) => {
        ...
        const colors = ['#76ef81', '#6db41d', '#009a73', '#801b8a', '#f6538e', '#67f1b2', '#f600b3', '#40E0D0', '#FFA500'];
        const datasets = foodLabels.map((food, index) => ({
            label: food,
            data: weeks.map((weekDates) => dailyLogs.filter(log => weekDates.includes(toDateKey(log.date)) && getFoodName(log) === food).reduce((sum, log) => sum + getKcal(log), 0)),
            backgroundColor: colors[index % colors.length]
        }));
        return { labels: weeks.map((_, i) => `Tuần ${i + 1}`), datasets };
    };
    ...
    const chartWrapperClass = "relative w-full h-[400px] mt-4";

    return (
        <div className="p-5 font-sans max-w-[1440px] mx-auto">
            ...
            <div className="mb-16">
                <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                    <p className="font-semibold text-sm">Filter theo tuần</p>
                    <select
                        value={selectedWeek}
                        onChange={(e) => setSelectedWeek(e.target.value)}
                        className="py-1.5 px-2.5 rounded-md border border-gray-300 font-medium text-sm outline-none w-max"
                    >
            ...
            <div className="mb-10">
                <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                    <p className="font-semibold text-sm">Filter theo tháng</p>
                    <select
                        value={selectedMonth}
